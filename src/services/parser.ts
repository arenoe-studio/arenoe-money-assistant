 ...
 * 3. Merge results
 */
export async function parseMessage(message: string): Promise<PartialTransaction> {
  // 1. Regex Parsing
  ...
  try {
    ...
    const merged: PartialTransaction = {
      ...
      metodePembayaran: (regexResult.metodePembayaran as any) || aiResult.metodePembayaran || undefined,
 ...
 */
function parseWithRegex(message: string): PartialTransaction {
  ...
  if (paymentMatch) {
    ...
    result.metodePembayaran = normalizePaymentMethod(method) as any;
  }
...
function isComplete(data: PartialTransaction): boolean {
