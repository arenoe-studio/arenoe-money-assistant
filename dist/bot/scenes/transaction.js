...
function getMissingFields(data) {
    ...
    if (!data.harga)
        missing.push('harga');
    if (!data.namaToko)
        missing.push('namaToko');
    if (!data.metodePembayaran)
        missing.push('metodePembayaran');
    return missing;
}
export const transactionScene = new Scenes.WizardScene(SCENE_ID, 
// Step 1
async (ctx) => {
    if (!ctx.session)
        ctx.session = {};
...
async (ctx) => {
    ...
    if (/^\d+$/.test(text)) {
        ...
        if (ctx.session.retryCount.merchant >= 2) {
            ...
            if (ctx.session.transaction)
                ctx.session.transaction.namaToko = 'Unknown';
    ...
    if (ctx.session.transaction)
        ctx.session.transaction.namaToko = text;
    if (!ctx.session.transaction?.metodePembayaran) {
...
async (ctx) => {
    ...
    if (!matched) {
        ...
        if (ctx.session.retryCount.payment >= 2) {
            ...
            if (ctx.session.transaction)
                ctx.session.transaction.metodePembayaran = 'Cash';
            ctx.wizard.selectStep(3);
            return;
        }
    ...
    if (ctx.session.transaction) {
        ...
        ctx.session.transaction.metodePembayaran = matched;
    }
    ctx.wizard.selectStep(3);
    return;
}, 
...
async (ctx) => {
    ...
    if (action === 'confirm_yes') {
        try {
            ...
            await writeTransaction(ctx.from.id, transaction);
